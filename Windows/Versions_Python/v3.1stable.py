...
class AppConfig:
    ...
    STYLE = {
        ...
        "font_large_title": ("Segoe UI", 26, "bold"),
...
def apply_windows_modern_blur(hwnd):
    """Applies modern Mica blur effect to the entire window on Windows 11."""
    ...
    try:
        ...
        dwmapi.DwmSetWindowAttribute(hwnd, DWMWA_SYSTEMBACKDROP_TYPE, ctypes.byref(value), ctypes.sizeof(value))

    except Exception as e:
        print(f"Failed to apply modern blur effect: {e}")
...
class NativeDialog(tk.Toplevel):
    def __init__(self, parent, title, message, dialog_type="info"):
        ...
        ttk.Label(main_frame, text=message, wraplength=360, justify="left", style='Dialog.TLabel').pack(expand=True, fill='both')
        button_frame = ttk.Frame(main_frame, style='Primary.TFrame'); button_frame.pack(side="bottom", fill="x", pady=(10, 0))
    ...
    def _add_yes_no_buttons(self, c):
        ...
        bc = ttk.Frame(c, style='Primary.TFrame'); bc.pack()
...
class StatusInfoDisplay(ttk.Frame):
    def __init__(self, master):
        ...
        self.status_label = ttk.Label(self, text="Initializing...", style='LightText.TLabel'); self.status_label.pack(fill='x', pady=(0, 2))
        self.info_label = ttk.Label(self, text="", style='ProgressInfo.TLabel'); self.info_label.pack(fill='x')
...
class AppGridViewPage(Page):
    ...
    class AppCard(ttk.Frame):
        def __init__(self, parent, app, controller):
            ...
            self.icon_label = ttk.Label(self, style='Card.TLabel')
...
class DetailsPage(Page):
    ...
    def _populate_view(self):
        ...
        about_title = ttk.Label(content_frame, text="About this App", style='Title.TLabel')
        about_title.pack(anchor="w", pady=(10, 5))

        web_container = ttk.Frame(content_frame, style='Primary.TFrame', padding=1)
        web_container.pack(fill="both", expand=True, pady=(0, 10))

        try:
            web_frame = HtmlFrame(web_container, messages_enabled=False, vertical_scrollbar=True)
            web_frame.load_website(self.app['website_url'])
            web_frame.pack(fill="both", expand=True)
        except Exception as e:
            error_label = ttk.Label(web_container, text=f"Could not load website preview.\nError: {e}", style='LightText.TLabel', wraplength=500)
            error_label.pack(pady=20, padx=20)
        
        ttk.Button(content_frame, text="Open in External Browser", command=lambda: self.controller.open_info_website(self.app)).pack(anchor="w", pady=5)
...
class AppStoreClient(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1700x1000"); self.minsize(800, 600)
        
        self.configure(bg=AppConfig.STYLE["background"])
        self.attributes("-transparentcolor", AppConfig.STYLE["background"])
        self.after(10, self.apply_blur)

        self.setup_styles()
        self.setup_icon()
        
        self.root_container = ttk.Frame(self, style='App.TFrame')
        ...
        app_container = ttk.Frame(self.root_container, style='App.TFrame'); 
        app_container.pack(fill="both", expand=True)
        
        nav_rail = ttk.Frame(app_container, width=220, style='Primary.TFrame'); nav_rail.pack(side="left", fill="y"); nav_rail.pack_propagate(False)
        self.content_area = ttk.Frame(app_container, style='App.TFrame'); self.content_area.pack(side="right", fill="both", expand=True)
    ...
    def apply_blur(self):
        if sys.platform == "win32":
            ...
            apply_windows_modern_blur(hwnd)
    ...
    def setup_icon(self):
        ...
        if not os.path.exists(icon_path):
            try:
                ...
                print(f"Error downloading app icon: {e}"); return
        if os.path.exists(icon_path):
            try: self.iconbitmap(icon_path)
            except Exception as e: print(f"Error setting app icon: {e}")

    def setup_styles(self):
        s = ttk.Style(); s.theme_use(AppConfig.STYLE["theme"]); S = AppConfig.STYLE
        
        s.configure('.', background=S["background"], foreground=S["text"], borderwidth=0, relief='flat')
        s.configure('App.TFrame', background=S["background"])
        s.configure('Primary.TFrame', background=S["primary"])
        s.configure('Card.TFrame', background=S["primary"])
        ...
        s.configure('Dialog.TLabel', background=S["primary"], foreground=S["text"], font=S["font_body"])
        s.configure('CardTitle.TLabel', background=S["primary"], foreground=S["text"], font=S["font_body_bold"])
        s.configure('ProgressInfo.TLabel', background=S["background"], foreground=S["accent"], font=S["font_nav"])

        s.configure('TButton', font=S["font_body_bold"], padding=10, relief='flat', borderwidth=0, background=S["secondary"])
        s.map('TButton', background=[('active', S["primary"])])
        s.configure('Accent.TButton', foreground='#000', background=S["accent"], font=S["font_body_bold"])
        s.map('Accent.TButton', background=[('active', S["accent_hover"]), ('disabled', S["disabled_bg"])], foreground=[('disabled', S["disabled_fg"])])
        
        s.configure('Nav.TButton', font=S["font_nav"], padding=(20, 12), background=S["primary"], foreground=S["text_secondary"], borderwidth=0)
        s.map('Nav.TButton', 
              background=[('active', S["secondary"]), ('selected', S["background"])], 
        ...
        s.configure('Vertical.TScrollbar', background=S["primary"], troughcolor=S["background"], borderwidth=0)
        s.map('Vertical.TScrollbar', background=[('active', S["secondary"])])

    def _create_navigation(self, nav_rail):
        ttk.Label(nav_rail, text="TIWUT", font=("Segoe UI", 22, "bold"), background=AppConfig.STYLE["primary"], foreground=AppConfig.STYLE["accent"]).pack(pady=25, padx=20)
        for text, page_name in [("Discover", "DiscoverPage"), ("My Library", "LibraryPage"), ("Settings", "SettingsPage")]:
            ...
            btn.pack(fill="x", padx=15, pady=4); self.nav_buttons[page_name] = btn
            ...
            try:
                ...
                for line in r.text.splitlines():
                    ...
                    if len(parts) >= 3:
                        ...
                        if app["name"] not in apps_dict: apps_dict[app["name"]] = app
            except requests.RequestException as e:
                errors.append(f"Could not load library from:\n{url}\nError: {e}\n")
        
        if errors: NativeDialog(self, "Network Error", "\n".join(errors))
        if not apps_dict and not errors: NativeDialog(self, "Library Empty", "No applications found in the configured libraries.")
    ...
    def reload_library(self):
        ...
        if isinstance(self.current_frame, DetailsPage):
            ...
            if updated_app_data: self.current_frame.on_show(app_data=updated_app_data, source_page=self.current_frame.source_page)
    ...
    def open_info_website(self, app):
        try: webbrowser.open(app['website_url'])
        except Exception as e: NativeDialog(self, "Error", f"Could not open website:\n{e}", "error")

    def create_shortcut(self, app):
        try:
            if sys.platform != 'win32':
                NativeDialog(self, "Unsupported", "Desktop shortcuts are only supported on Windows.", "error"); return
            ...
            with open(script_path, 'w', encoding='utf-8') as f:
                ...
                sLinkFile = "{shortcut_path.replace('\\', '\\\\')}"
                Set oLink = oWS.CreateShortcut(sLinkFile)
                oLink.TargetPath = "{target_path.replace('\\', '\\\\')}"
                oLink.WorkingDirectory = "{working_dir.replace('\\', '\\\\')}"
                oLink.IconLocation = "{target_path.replace('\\', '\\\\')}, 0"
                oLink.Save
                ''')
    ...
    def install_app(self, app):
        ...
        if dp.progress_widget and dp.progress_widget.winfo_exists():
            return 
        dp.progress_widget = StatusInfoDisplay(dp.progress_container)
        dp.progress_widget.pack(fill='x')
    ...
    def _download_task(self, app):
        dp = self.frames["DetailsPage"]
        progress_widget = dp.progress_widget
        try:
            self.after(0, lambda: progress_widget.update_full(0, "Connecting...", ""))
            ...
            with requests.get(app["download_url"], stream=True, timeout=15) as r:
                ...
                with open(zip_path, "wb") as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        ...
                        p = 0
                        if total_size > 0:
                            p = (downloaded_size / total_size) * 100
                            info = f"{format_bytes(downloaded_size)} / {format_bytes(total_size)}"
                        else:
                            info = f"{format_bytes(downloaded_size)} downloaded"
                        
                        self.after(0, lambda pct=p, i=info: progress_widget.update_full(pct, f"Downloading {app['name']}...", i))

            self.after(0, lambda: progress_widget.update_full(100, "Extracting files...", ""))
            ...
            if progress_widget and progress_widget.winfo_exists():
                self.after(0, progress_widget.destroy)
