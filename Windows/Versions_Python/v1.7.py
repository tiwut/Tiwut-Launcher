...
class ScrollableFrame(tk.Frame):
    def __init__(self, container, *args, **kwargs):
        super().__init__(container, *args, **kwargs); self.config(bg=STYLE["background"]); self.canvas = tk.Canvas(self, bg=STYLE["background"], highlightthickness=0); self.scrollable_frame = tk.Frame(self.canvas, bg=STYLE["background"]); self.scrollable_frame.bind("<Configure>", lambda e: self.canvas.configure(scrollregion=self.canvas.bbox("all"))); self.canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw"); self.canvas.pack(side="left", fill="both", expand=True); self.bind('<Enter>', self._bind_to_mousewheel); self.bind('<Leave>', self._unbind_from_mousewheel)
    def _bind_to_mousewheel(self, e): self.canvas.bind_all("<MouseWheel>", self._on_mousewheel)
    def _unbind_from_mousewheel(self, e): self.canvas.unbind_all("<MouseWheel>")
...
class TiwutLauncher(tk.Tk):
    ...
    def _create_navigation(self, active_page):
        ...
        self.home_button = RoundedButton(nav_frame, "My Apps", self.create_home_page, 200, 50, font=STYLE["font_nav"])
        self.home_button.pack(side="left", padx=10)
        self.discover_button = RoundedButton(nav_frame, "Discover", self.create_discover_page, 200, 50, font=STYLE["font_nav"])
        self.discover_button.pack(side="left", padx=10)
        if active_page == "home":
            self.home_button.set_state("active")
        elif active_page == "discover":
            self.discover_button.set_state("active")
