...
def apply_windows_dark_title_bar(hwnd):
...
class DetailsPage(Page):
    ...
    def _populate_view(self):
        ...
        web_container = ttk.Frame(content_frame, style='Card.TFrame', padding=1)
        web_container.pack(fill="both", expand=True, pady=(0, 10))
        web_container.configure(borderwidth=1, relief='solid')
...
class AppStoreClient(tk.Tk):
    def __init__(self):
        ...
        self.after(10, self.set_dark_mode_title_bar)
        self.geometry("1700x1100"); self.minsize(800, 600)
        ...
        nav_rail = ttk.Frame(app_container, width=200, style='Primary.TFrame'); nav_rail.pack(side="left", fill="y"); nav_rail.pack_propagate(False)
    ...
    def set_dark_mode_title_bar(self):
        if sys.platform == "win32":
    ...
    def setup_styles(self):
        ...
        s.configure('TButton', font=S["font_body_bold"], padding=10, relief='flat', borderwidth=0)
        s.map('TButton', background=[('active', S["secondary"])])
        s.configure('Accent.TButton', foreground=S["background"], background=S["accent"], font=S["font_body_bold"])
    ...
    def install_app(self, app):
        ...
        if dp.progress_widget and dp.progress_widget.winfo_exists():
            return 
        dp.progress_widget = StatusProgressBar(dp.progress_container)
