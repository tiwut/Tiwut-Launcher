...
import shutil
import webview
import time # Importieren wir das time-Modul für die Geschwindigkeitsmessung
...
# --- NEUER, verbesserter Fortschrittsbalken ---
...
# --- Helper zum Formatieren von Bytes in KB/MB/GB ---
...
# --- Restliche Klassen (Button, Dialog) bleiben unverändert ---
class RoundedButton(tk.Frame):
    def __init__(self, master, text, command, width, height, radius=25):
        super().__init__(master, width=width, height=height, bg=STYLE["background"]); self.command = command; self.is_disabled = False
        self.canvas = tk.Canvas(self, width=width, height=height, bg=STYLE["background"], highlightthickness=0); self.canvas.pack()
        ...
        self.canvas.bind("<Button-1>", self._on_click); self.canvas.bind("<Enter>", self._on_hover); self.canvas.bind("<Leave>", self._on_leave)
...
class CustomDialog(tk.Toplevel):
    ...
    def askyesno(self):
        ...
        RoundedButton(self.button_frame, "Yes", on_yes, width=100, height=50).pack(side="left", padx=10); RoundedButton(self.button_frame, "No", on_no, width=100, height=50).pack(side="left", padx=10)
...
class TiwutLauncher(tk.Tk):
    def __init__(self):
        ...
        self.bg_canvas = tk.Canvas(self, bg=STYLE["transparent_color"], highlightthickness=0); self.bg_canvas.pack(fill="both", expand=True)
        self.bind("<Configure>", self.draw_background)
        self.content_frame = tk.Frame(self, bg=STYLE["background"]); self.bg_canvas.create_window(0, 0, anchor="nw", window=self.content_frame)
        self.title_bar = tk.Frame(self.content_frame, bg=STYLE["widget_bg"], relief='raised', bd=0); self.title_bar.pack(expand=0, fill='x')
    ...
    def draw_background(self, event=None):
        self.bg_canvas.delete("all"); width, height = self.winfo_width(), self.winfo_height()
    ...
    def load_library_from_url(self):
        ...
        try:
            ...
            for line in r.text.splitlines():
                if line.strip(): parts = line.strip().split(";");
    ...
    def create_home_page(self):
        self.clear_main_frame(); container = tk.Frame(self.main_frame, bg=STYLE["background"]); container.pack(pady=40, padx=20, fill="both", expand=True)
        ...
        search_frame = tk.Frame(container, bg=STYLE["background"]); search_frame.pack(pady=10, fill='x', padx=50)
        search_canvas = tk.Canvas(search_frame, height=50, bg=STYLE["background"], highlightthickness=0); search_canvas.pack(fill='x')
        self.search_var = tk.StringVar(); search_entry = tk.Entry(search_frame, textvariable=self.search_var, font=STYLE["font_main"], bg=STYLE["widget_bg"], fg=STYLE["foreground"], insertbackground=STYLE["primary_neon"], relief=tk.FLAT, borderwidth=0)
        def on_frame_configure(e): search_canvas.delete("all"); create_rounded_rect(search_canvas, 0, 0, e.width, e.height, 25, fill=STYLE["widget_bg"], outline=STYLE["secondary_neon"], width=2); search_entry.place(relx=0.5, rely=0.5, anchor='center', relwidth=0.9)
        search_frame.bind("<Configure>", on_frame_configure); self.search_var.trace("w", self.search_apps)
        self.app_list_frame = tk.Frame(container, bg=STYLE["background"]); self.app_list_frame.pack(pady=20, fill="both", expand=True); self.display_random_apps()
    def display_random_apps(self):
        ...
        for app in random.sample(self.apps, min(len(self.apps), 5)): RoundedButton(self.app_list_frame, app["name"], lambda a=app: self.show_app_details(a), width=300, height=50).pack(pady=8)
    ...
    def show_app_details(self, app):
        self.clear_main_frame(); container = tk.Frame(self.main_frame, bg=STYLE["background"]); container.pack(pady=20, padx=20, fill="both", expand=True)
        ...
        self.install_button = RoundedButton(button_frame, "Install", lambda: self.install_app(app), width=150, height=50); self.uninstall_button = RoundedButton(button_frame, "Uninstall", lambda: self.uninstall_app(app), width=150, height=50); self.open_button = RoundedButton(button_frame, "Open", lambda: self.open_app(app), width=150, height=50); self.info_button = RoundedButton(button_frame, "Description", lambda: self.open_info_website(app), width=150, height=50)
        ...
        self.update_buttons(is_installed); tk.Label(container, text="Click 'Description' to open app details in a new window.", font=STYLE["font_main"], bg=STYLE["background"], fg=STYLE["secondary_neon"], wraplength=500).pack(pady=40)
    ...
    def open_info_website(self, app):
        ...
        except Exception as e: CustomDialog(self, "Error", f"Could not open window: {e}").showerror()

    def install_app(self, app):
        self.progress_bar = ProgressBar(self.main_frame); self.progress_bar.pack(fill='x', side='bottom')
            ...
            with requests.get(app["download_url"],stream=True)as r:
                ...
                with open(zip_path,"wb")as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        ...
                        # Update der GUI nur alle 0.25 Sekunden, um sie nicht zu überlasten
                        if total_size > 0 and (current_time - last_update_time) > 0.25:
                            progress = (downloaded / total_size) * 100
                            elapsed_time = current_time - last_update_time
                            speed = downloaded_since_update / elapsed_time
                            
                            status = "Downloading..."
                            info = f"{format_bytes(speed)}/s  |  {format_bytes(downloaded)} / {format_bytes(total_size)}"
                            
                            # GUI-Update sicher aus dem Thread aufrufen
                            self.after(0, self.progress_bar.update_full, progress, status, info)
                            
                            # Zähler zurücksetzen
                            last_update_time = current_time; downloaded_since_update = 0

            # Update vor der Extraktion
            self.after(0, self.progress_bar.update_full, 100, "Extracting...", "Please wait...")
