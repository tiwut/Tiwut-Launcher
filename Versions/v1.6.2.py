...
class AnimatedProgressBar(tk.Frame):
    def __init__(self, master, height=40):
        ...
        self.canvas = tk.Canvas(self, height=height, bg=STYLE["background"], highlightthickness=0)
        self.canvas.pack(fill='x', expand=True, padx=50, pady=(20, 10))
        self.status_text_id = self.canvas.create_text(0, 0, text="Initializing...", font=("Consolas", 12), fill=STYLE["secondary_neon"])
        self.info_text_id = self.canvas.create_text(0, 0, text="", font=("Consolas", 10), fill=STYLE["tertiary_neon"])
    ...
    def _on_resize(self, event=None):
        ...
        self.bg_rect = create_rounded_rect(self.canvas, 2, height - 22, width - 2, height - 2, radius=10, fill=STYLE["widget_bg"], width=0)
        self.progress_rect = create_rounded_rect(self.canvas, 2, height - 22, 2, height - 2, radius=10, fill=STYLE["tertiary_neon"], width=0)
        self.status_text_id = self.canvas.create_text(width / 2, 10, text=self._target_status, font=("Consolas", 12), fill=STYLE["secondary_neon"])
        self.info_text_id = self.canvas.create_text(width / 2, height - 12, text=self._target_info, font=("Consolas", 10), fill=STYLE["tertiary_neon"])
    def update_full(self, percentage, status_text, info_text):
        self.target_percentage = percentage; self._target_status = status_text; self._target_info = info_text
    def _animate_progress(self):
        ...
        if width > 10:
            ...
            self.canvas.coords(self.progress_rect, 2, height - 22, max(2, new_width), height - 2)
            self.canvas.itemconfig(self.status_text_id, text=self._target_status)
            self.canvas.itemconfig(self.info_text_id, text=self._target_info)
...
class RoundedButton(tk.Frame):
    def __init__(self, master, text, command, width, height, radius=25):
        ...
        self.command = command; self.is_disabled = False; self.canvas = tk.Canvas(self, width=width, height=height, bg=STYLE["background"], highlightthickness=0); self.canvas.pack()
...
class TiwutLauncher(tk.Tk):
    ...
    def create_home_page(self):
        ...
        tk.Label(container, text="T I W U T", font=STYLE["font_title"], bg=STYLE["background"], fg=STYLE["primary_neon"]).pack(pady=(0, 20))
        ...
        def on_frame_configure(e):
            search_canvas.delete("all"); create_rounded_rect(search_canvas, 0, 0, e.width, e.height, 25, fill=STYLE["widget_bg"], outline=STYLE["secondary_neon"], width=2); self.search_entry.place(relx=0.5, rely=0.5, anchor='center', relwidth=0.9)
    ...
    def search_apps(self, *args):
        ...
        if not search_term or search_term == "type to search...":
            self.display_random_apps()
            return
    ...
    def _populate_app_list(self, apps_to_show, title):
        ...
        if not apps_to_show:
            tk.Label(frame, text="No results found.", font=STYLE["font_main"], bg=STYLE["background"], fg=STYLE["foreground"]).pack()
            return
        # Frame to center the buttons
        center_frame = tk.Frame(frame, bg=STYLE["background"])
        center_frame.pack(expand=True)
        for app in apps_to_show:
            RoundedButton(center_frame, app["name"], lambda a=app: self.show_app_details(a), 450, 50).pack(pady=8)
    ...
    def show_app_details(self, app):
        ...
        self.install_button.pack(side=tk.LEFT, padx=10); self.uninstall_button.pack(side=tk.LEFT, padx=10)
        self.open_button.pack(side=tk.LEFT, padx=10); self.info_button.pack(side=tk.LEFT, padx=10)
    ...
    def install_app(self, app):
        self.progress_bar = AnimatedProgressBar(self.main_frame)
    ...
    def _download_and_extract(self, app):
        try:
            ...
            with requests.get(app["download_url"], stream=True) as r:
                ...
                with open(zip_path, "wb") as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        if chunk: f.write(chunk); downloaded += len(chunk); downloaded_since_update += len(chunk)
            ...
            self.after(0, self.progress_bar.update_full, 100, "Extracting Files...", "Unpacking, please wait...")
            time.sleep(0.5)
