...
class TiwutLauncher(tk.Tk):
    ...
    def _create_title_bar(self):
        title_bar = tk.Frame(self.content_frame, bg=STYLE["widget_bg"], bd=0)
        title_bar.pack(expand=0, fill='x')
        tk.Label(title_bar, text="Tiwut Launcher", bg=STYLE["widget_bg"], fg=STYLE["secondary_neon"], font=("Consolas", 10)).pack(side='left', padx=10)
        tk.Button(title_bar, text='X', command=self.destroy, bg=STYLE["widget_bg"], fg=STYLE["primary_neon"], relief="flat", font=("Consolas", 10, "bold")).pack(side='right', padx=5)
    ...
    def _draw_background(self, event=None):
        ...
        create_rounded_rect(self.bg_canvas, 0, 0, w, h, 30, fill=STYLE["background"])
        self.bg_canvas.create_window(0, 0, anchor="nw", window=self.content_frame, width=w, height=h)
    ...
    def _create_navigation(self, active_page):
        nav_frame = tk.Frame(self.main_frame, bg=STYLE["background"])
        nav_frame.pack(pady=20)
        home_button = RoundedButton(nav_frame, "My Apps", self.create_home_page, 200, 50, font=STYLE["font_nav"])
        home_button.pack(side="left", padx=10)
        discover_button = RoundedButton(nav_frame, "Discover", self.create_discover_page, 200, 50, font=STYLE["font_nav"])
        discover_button.pack(side="left", padx=10)
        if active_page == "home": home_button.set_state("active")
        elif active_page == "discover": discover_button.set_state("active")

    def _create_search_bar(self, var):
        sc = tk.Frame(self.main_frame, bg=STYLE["background"])
        sc.pack(pady=10, fill='x', padx=200)
        s_cv = tk.Canvas(sc, height=50, bg=STYLE["background"], highlightthickness=0)
        s_cv.pack(fill='x')
        s_entry = tk.Entry(sc, textvariable=var, font=STYLE["font_main"], bg=STYLE["widget_bg"], fg=STYLE["foreground"], insertbackground=STYLE["primary_neon"], relief=tk.FLAT, bd=0)
        ph = "Type to search..."
        s_entry.insert(0, ph)
    ...
    def create_home_page(self):
        ...
        self.s_var_home = tk.StringVar()
        self.s_var_home.trace("w", self._search_installed)
        self._create_search_bar(self.s_var_home)
        self.installed_list_frame = ScrollableFrame(self.main_frame)
        self.installed_list_frame.pack(pady=10, fill="both", expand=True, padx=50)
        self._populate_installed_list()

    def create_discover_page(self):
        self.clear_main_frame()
        self._create_navigation("discover")
        ...
        self.s_var_discover = tk.StringVar()
        self.s_var_discover.trace("w", self._search_discover)
        self._create_search_bar(self.s_var_discover)
        self.app_list_frame = ScrollableFrame(self.main_frame)
        self.app_list_frame.pack(pady=10, fill="both", expand=True, padx=50)
        self._populate_discover_list(random.sample(self.apps, min(len(self.apps), 15)), "Featured Apps")

    def _search_installed(self, *args):
        if not hasattr(self, 'installed_list_frame'): return
        search_term = self.s_var_home.get().lower()
        if not search_term or search_term == "type to search...": self._populate_installed_list(); return
        results = [app for app in self._get_installed_apps() if search_term in app["name"].lower()]
        self._populate_installed_list(results, "Search Results")
    
    def _search_discover(self, *args):
        if not hasattr(self, 'app_list_frame'): return
        search_term = self.s_var_discover.get().lower()
        ...
        self._populate_discover_list(results, "Search Results")

    def _populate_discover_list(self, apps, title):
        ...
        for app in apps: RoundedButton(center_frame, app["name"], lambda a=app: self.show_app_details(a, "discover"), 600, 50).pack(pady=8)
    
    def _get_installed_apps(self): return [app for app in self.apps if os.path.exists(os.path.join(os.getcwd(), app["name"]))]
    
    def _populate_installed_list(self, apps=None, title=None):
        ...
        apps_to_show = apps if apps is not None else self._get_installed_apps()
        ...
        center_frame = tk.Frame(frame, bg=STYLE["background"]); center_frame.pack(expand=True)
        for app in apps_to_show: RoundedButton(center_frame, app["name"], lambda a=app: self.show_app_details(a, "home"), 600, 50).pack(pady=8)
    
    def show_app_details(self, app, source_page):
        self.clear_main_frame()
        container = tk.Frame(self.main_frame, bg=STYLE["background"]); container.pack(pady=20, padx=20, fill="both", expand=True)
        back_cmd = self.create_home_page if source_page == "home" else self.create_discover_page
        back_text = "< Back to My Apps" if source_page == "home" else "< Back to Discover"
        RoundedButton(container, back_text, back_cmd, 250, 50).pack(anchor="nw", pady=(0, 20))
        tk.Label(container, text=app["name"], font=("Consolas", 36, "bold"), bg=STYLE["background"], fg=STYLE["primary_neon"]).pack(pady=20)
        bf = tk.Frame(container, bg=STYLE["background"]); bf.pack(pady=20)
        is_inst = os.path.exists(os.path.join(os.getcwd(), app["name"]))
        self.install_b = RoundedButton(bf, "Install", lambda: self.install_app(app), 150, 50)
        self.uninstall_b = RoundedButton(bf, "Uninstall", lambda: self.uninstall_app(app), 150, 50)
        self.open_b = RoundedButton(bf, "Open", lambda: self.open_app(app), 150, 50)
        self.info_b = RoundedButton(bf, "View Details", lambda: self.open_info_website(app), 150, 50)
        self.install_b.pack(side=tk.LEFT, padx=10); self.uninstall_b.pack(side=tk.LEFT, padx=10); self.open_b.pack(side=tk.LEFT, padx=10); self.info_b.pack(side=tk.LEFT, padx=10)
        self.update_buttons(is_inst)
