# Tiwut App Store Client v2.8
#
# Changes in this version:
# 1. New Feature: Added a "Create Shortcut" button for installed apps on Windows.
# 2. Bug Fix: Implemented a more robust icon loading system to ensure icons are
#    always displayed correctly, even when the app grid is updated quickly.
...
class AppStoreClient(tk.Tk):
    ...
    def create_shortcut(self, app):
        try:
            ...
            icon_path_vbs = target_path_vbs # Default icon is the exe itself
            ...
            with open(script_path, 'w') as f: f.write(vbs_script)
    ...
    def _download_task(self, app):
        ...
        try:
            ...
            os.remove(zip_path)
            self.after(0, lambda: [
                self.frames["DetailsPage"].on_show(app_data=app, source_page=self.get_page_name(self.current_frame)), 
